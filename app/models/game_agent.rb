...
class GameAgent < ApplicationRecord
  ...
  def call(input, context_items: [], &block)
    ...
    transaction do
      ...
      chat = RubyLLM.chat(model: game.user.preferred_model)
      chat.with_instructions(context_string)
      chat.with_tools(*tools)

      # Load conversation history into chat (excluding current message)
      history = conversation_history[0...-1] # All but the last message we just added
      history.each do |msg|
        case msg["role"]
        when "user"
      ...
      if stream
        ...
        response = chat.ask(input) do |chunk|
          yield({ type: "content", content: chunk.content }) if chunk.content.present?
        end
      else
        # Non-streaming mode
